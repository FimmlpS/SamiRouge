...
import SamiRouge.monsters.alts.ThreeGremlinLeader;
...
public class SpecialSummonGremlinAction extends AbstractGameAction {
    private static final Logger logger = LogManager.getLogger(SpecialSummonGremlinAction.class.getName());
    private AbstractMonster m;

    public SpecialSummonGremlinAction(AbstractMonster[] gremlins) {
    ...
    private AbstractMonster getRandomGremlin(int slot) {
        ...
        switch(slot) {
            case 0:
                x = ThreeGremlinLeader.POSX[0];
                y = ThreeGremlinLeader.POSY[0];
                break;
            case 1:
                x = ThreeGremlinLeader.POSX[1];
                y = ThreeGremlinLeader.POSY[1];
                break;
            case 2:
                x = ThreeGremlinLeader.POSX[2];
                y = ThreeGremlinLeader.POSY[2];
                break;
            case 3:
                x = ThreeGremlinLeader.POSX[3];
                y = ThreeGremlinLeader.POSY[3];
                break;
            default:
                x = ThreeGremlinLeader.POSX[0];
                y = ThreeGremlinLeader.POSY[0];
